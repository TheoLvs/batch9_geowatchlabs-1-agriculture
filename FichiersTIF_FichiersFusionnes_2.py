...
pixel=2 # Nb pixels autour du menage
for i in range(len(liHistYield)) :
    ...
    df_analyse.loc[:,"HY_Rend_"+temp] = Calcul_Rend(pixel,liHistYield_np[i],df_analyse["HY_row_"+temp],df_analyse["HY_col_"+temp],df_analyse,max_row,max_col)
...
for i in range(len(liCropMask)) :
    temp=liCropMask[i][0]+"_"+liCropMask[i][1] # annee + prod + zone
    ...
    df_analyse.loc[:,"CM_Rend_"+temp] = Calcul_Rend(pixel,liCropMask_np[i],df_analyse["CM_row_"+temp],df_analyse["CM_col_"+temp],df_analyse,max_row,max_col)
